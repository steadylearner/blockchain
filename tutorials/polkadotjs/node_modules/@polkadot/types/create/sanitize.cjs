...
exports.removeBoundedVec = removeBoundedVec;
...
removeBoundedVec(), // Remove all the trait prefixes
...
alias('Vec<u8>', 'Bytes'), alias('&\\[u8\\]', 'Bytes'), // alias RawAddress -> Address
...
function removeBoundedVec() {
  const replacer = inner => `Vec<${inner.split(',')[0]}>`;

  return value => replaceTagWith(value, 'BoundedVec<', replacer);
}
